...
/* 팔로우 추가 API */
followsRouter.post('/:followeeId', accessMiddleware, catchError(async (req, res) => {
...
/* 팔로워 목록 조회 API */
followsRouter.get('/followers/:userId', catchError(async (req, res) => {
...
/* 팔로잉 목록 조회 API */
followsRouter.get('/following/:userId', catchError(async (req, res) => {
  const userId = parseInt(req.params.userId, 10);
...
/* 팔로우 삭제 API */
followsRouter.delete('/:followeeId', accessMiddleware, catchError(async (req, res) => {
  const followeeId = parseInt(req.params.followeeId, 10);
  const followerId = req.user.id;
